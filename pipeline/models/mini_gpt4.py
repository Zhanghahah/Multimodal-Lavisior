...
class MiniGPT4(BaseModel):
    ...
    def encode_img(self, inputs, device, do_proj=True):
        ...
        if "gnn" in self.encoder_names:
            ...
            if not self.use_graph_agg:
                graph_feat = self.pad_node(graph, graph_feat)  # torch.Size([1, 11, 300])
            feat = graph_feat
            inputs["feat"] = feat
    ...
    def proj_feat(self, features, device):
        ...
        embeds = self.ln_vision(features["feat"]).to(device) # torch.Size([1, 11, 300])

        inputs_llama = self.llama_proj(embeds)  # torch.Size([1, 11, 4096])
        atts_llama = torch.ones(inputs_llama.size()[:-1], dtype=torch.long).to(device) # torch.Size([1, 11])
        return inputs_llama, atts_llama
    ...
    def prompt_wrap(self, img_embeds, atts_img, prompts):
        if prompts:
            ...
            p_before_embeds = self.llama_model.model.embed_tokens(p_before_tokens.input_ids)#.expand(batch_size, -1, -1) torch.Size([1, 8, 4096])
            p_after_embeds = self.llama_model.model.embed_tokens(p_after_tokens.input_ids)#.expand(batch_size, -1, -1)   torch.Size([1, 15, 4096])
            if self.soft_prompt is not None:
                img_embeds = torch.cat([img_embeds, self.soft_prompt.expand(batch_size, -1, -1)], 1)
            wrapped_img_embeds = torch.cat([p_before_embeds, img_embeds, p_after_embeds], dim=1) # torch.Size([1, 34, 4096])
            wrapped_atts_img = atts_img[:, :1].expand(-1, wrapped_img_embeds.shape[1])  # wrapped_atts_img 1,34
